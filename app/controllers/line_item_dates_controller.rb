  ...
  def create
    ...
    if @line_item_date.save
      respond_to do |format|
        format.html { respond_to quote_path(@quote), notice: "Date added!"}
        format.turbo_stream { flash.now[:notice]="Date added!"}
      end
    else
  ...
  def update
    if @line_item_date.update(line_item_date_params)
      respond_to do |format|
        format.html { redirect_to quote_path(@quote), notice: "Date updated"}
